...
**Created:** 2026-09-03 | **Last updated:** 2026-09-03 — pass 2 (modernization) | **Author:** research agent under Axium (R2) | **Status:** exploratory — not doctrine
...
- **On an isolated Kubernetes cluster, a realistic first-release fabric is small**: one OSS catalog (OpenMetadata or DataHub), Postgres-backed domain stores, Kafka + Debezium for change events, Trino over Iceberg on S3-compatible object storage for read-side federation, and OPA for policy — all Apache-2.0, all Helm-installable from a mirrored registry. **One 2026 correction:** the MinIO community edition this class of design used to assume is no longer maintained (its repository README says so; the community build is source-only, AGPLv3 — verified 2026-09-03), so the object-storage slot needs a fresh choice (§5.2). The "enterprise" picture (knowledge graph, AI-driven active metadata, virtualization suite) is a later, deliberate step.
- **What the front end builds**: catalog/discovery, query/preview, security-marking display on every data-bearing component, and lineage/provenance views. Those four surfaces *are* the fabric to a user — and each is a signal-first, zoneless Angular 22 surface fed by `httpResource`/`resource` or a SignalStore, not a smart/dumb component pair (§7).
...
**Current majors (verified 2026-09-03 against each project's own repository; exact files in the Modernization ledger).**

| Tool | Current release | Where verified | Note for RR |
|---|---|---|---|
| OpenMetadata | **2.0.1** (`2.0.1-release` tag; `main` is 2.0.0-SNAPSHOT; PyPI `openmetadata-ingestion` 2.0.1.0, 2026-09-02) | `open-metadata/OpenMetadata` | 2.0 is a new major — read its upgrade notes before pinning. |
| DataHub | **v1.7.0** (tag; PyPI `acryl-datahub` 1.7.0.9, 2026-09-03) | `datahub-project/datahub` | Helm chart version not re-checked. |
| Trino | **483** (17 Jul 2026; `master` is 484-SNAPSHOT) | `trinodb/trino` release notes | Near-weekly cadence; pin one release and mirror its connector jars. |
| Apache Iceberg | **1.11.0** (19 May 2026) | `apache/iceberg` `site/docs/releases.md` | `main` defaults to Spark 4.1 / Flink 2.3. |
| Debezium | **3.6.0.Final** (1 Jul 2026); 3.7 at Beta1 | `debezium/debezium` `CHANGELOG.md` | |
| Open Policy Agent | **1.20.1** (`main` is 1.21.0-dev) | `open-policy-agent/opa` `CHANGELOG.md` | OPA 1.x defaults to Rego v1 syntax `[UNVERIFIED — 1.0 notes not fetched]`. |
| MinIO | **Not a candidate.** Repository README: "THIS REPOSITORY IS NO LONGER MAINTAINED"; the community edition is source-only, AGPLv3; MinIO points to AIStor Free (free licence, not open source) and AIStor Enterprise. | `minio/minio` `README.md`, `LICENSE` | S3-compatible slot re-opened: Apache Ozone (Apache-2.0), SeaweedFS (Apache-2.0), Ceph RGW `[UNVERIFIED — not fetched]`, or the island's existing object store. Decide with Q1 in §8. |
...
**A small, honest first-release fabric.** (1) One catalog. (2) Domain Postgres stores, one per bounded context, with *published views* registered in the catalog. (3) Debezium → Kafka → Iceberg on S3-compatible object storage (a candidate still to be chosen — §5.2) for analytical copies. (4) Trino for read-side SQL over Iceberg and published views. (5) OPA as the single PDP, fed by catalog tags, enforced at the Express gateway, in Trino, and mirrored in the UI. (6) Glossary maintained by context owners. No knowledge graph, no AI-driven active metadata, no commercial virtualization suite. That is legitimately a data fabric in the JADC2 sense — federated, discoverable, tagged, interface-based — without the enterprise price.
...
**The front end.** The workstation's four fabric surfaces are signal-first, zoneless Angular 22 surfaces (`@angular/core` 22.1.5, `@ngrx/signals` 22.0.0 — npm registry, 2026-09-03): standalone `OnPush` components (the v22 default), `@if`/`@for`/`@defer` control flow, no NgModules, no `@Input()` decorators, no component-owned RxJS subscriptions, and no smart/dumb split — signal `input()`s plus stores plus `resource`s replace it.

- **Catalog/discovery** — search, facets by domain/owner/classification, asset detail with schema and quality — reads OpenMetadata's or DataHub's REST/GraphQL APIs *through the Express gateway*, never directly. The search request is an `httpResource` whose request object is computed from the filter signals (`() => ({ url: '/api/catalog/assets', params: { q: query(), owner: owner() } })`), so a facet change re-issues the request and cancels the stale one; assets, the selected asset id and facet state live in a `CatalogStore` (`signalStore` with `withEntities` for assets, `withComputed` for the visible slice, `withMethods`/`patchState` for selection). Templates gate on `assets.hasValue()` / `assets.isLoading()` / `assets.error()`; the heavy detail panel sits in a `@defer (on viewport)` block.
- **Query/preview** — Trino-backed, read-only, paginated. A page is a `resource({ params, loader })` keyed on the statement and cursor signals, with the loader honouring `abortSignal`; the cursor is a `linkedSignal` that resets when the statement changes. Result rows carry their markings from the API — the surface never computes a marking client-side.
- **Marking display** — an AstroUXDS-styled classification banner on every data-bearing component, driven by the same tag payload the PDP evaluated, rendered by the R5 marking primitives (signal `input()` of a `Marking` value object; vocabulary loaded through a `resource`; `@if`-gated so nothing paints before the vocabulary resolves).
- **Lineage/provenance** — both catalogs expose lineage APIs; fetch the graph with an `httpResource` keyed on the selected asset and hand its `value()` to a graph library inside a `@defer` block, not to hand-rolled SVG.

Presumed-global state — selected asset, active markings, the subject's clearance/attributes — lives in root-provided SignalStores, not in components; per-surface transient state (facet drafts, cursor) stays surface-local and is declared so in the Concordance. Forms (saved-query editor, facet forms) use Signal Forms (`@angular/forms/signals`: public API in v22, experimental in v21, absent before). `HttpClient` is provided once at bootstrap (`provideHttpClient()`; v22 defaults to the Fetch backend) and reached only through `httpResource`/`rxResource` or a store `rxMethod`. **Version sensitivity for the Legacy Island re-pin (v19–v22):** `input()`/`output()`/`model()` stable since v19; `linkedSignal` and zoneless stable since v20 (zoneless the default from v21); `resource`/`httpResource`/`rxResource` experimental in v19.2–v21 and public API in v22.0; `OnPush` the default only in v22 (declare it explicitly on v19–v21); Signal Forms public in v22 only — all verified against `angular/angular` `CHANGELOG.md` and `adev/src/content/guide/**` on 2026-09-03.
...
### Concept sources (any age — cited for the idea, not for a version)
...
- OpenTDF specification (concept: manifest + encrypted payload + bound ABAC policy) — https://github.com/opentdf/spec/blob/main/README.md ; platform — https://github.com/opentdf/platform (versions: R5/R6 ledgers)
...
- Apache Ranger tag-based policies (concept) — https://cwiki.apache.org/confluence/display/RANGER/Tag+based+policy+requirements

### Implementation / idiom sources (dated, primary — fetched 2026-09-03 unless marked)

- Angular `adev` guides (repo `angular/angular`, branch `main`): *Async reactivity with resources* — https://github.com/angular/angular/blob/main/adev/src/content/guide/signals/resource.md ; *Reactive data fetching with `httpResource`* — https://github.com/angular/angular/blob/main/adev/src/content/guide/http/http-resource.md ; *Angular without ZoneJS* — https://github.com/angular/angular/blob/main/adev/src/content/guide/zoneless.md ; *Forms with Angular Signals* — https://github.com/angular/angular/blob/main/adev/src/content/guide/forms/signals/overview.md ; *Accepting data with input properties* — https://github.com/angular/angular/blob/main/adev/src/content/guide/components/inputs.md ; `CHANGELOG.md` (22.0.0 / 21.0.0 / 20.0.0 / 19.2.0 / 19.0.0 entries) — https://github.com/angular/angular/blob/main/CHANGELOG.md ; stability tags `@publicApi 22.0` — https://github.com/angular/angular/blob/main/packages/core/src/resource/resource.ts , https://github.com/angular/angular/blob/main/packages/common/http/src/resource.ts
- NgRx SignalStore guides (repo `ngrx/platform`, `main`): https://github.com/ngrx/platform/tree/main/projects/www/src/app/pages/guide/signals/signal-store (`index`, `linked-state`, `events`, `custom-store-features`); `modules/signals/package.json` (22.0.0, peer `@angular/core ^22.0.0`)
- npm registry (2026-09-03): `@angular/core` 22.1.5 (published 2026-09-03), `@ngrx/signals` 22.0.0 (2026-08-24), `@astrouxds/angular` 9.0.0 (2026-06-23) — https://registry.npmjs.org/
- OpenMetadata 2.0.1 — https://github.com/open-metadata/OpenMetadata (tag `2.0.1-release`; PyPI `openmetadata-ingestion` 2.0.1.0) ; docs — https://docs.open-metadata.org/ `[UNVERIFIED — host blocked]`
- DataHub v1.7.0 — https://github.com/datahub-project/datahub (tag `v1.7.0`; PyPI `acryl-datahub` 1.7.0.9) ; Helm — https://github.com/acryldata/datahub-helm `[chart version not re-checked]`
- Apache Atlas — https://github.com/apache/atlas ; Apache Ranger — https://github.com/apache/ranger `[versions not re-checked in pass 2 — deferred candidates]`
- Trino 483 (17 Jul 2026) — https://github.com/trinodb/trino/blob/master/docs/src/main/sphinx/release/release-483.md ; concepts — https://trino.io/docs/current/overview/concepts.html `[UNVERIFIED — host blocked]`
- Apache Iceberg 1.11.0 (19 May 2026) — https://github.com/apache/iceberg/blob/main/site/docs/releases.md ; spec — https://iceberg.apache.org/spec/ `[UNVERIFIED — host blocked]`
- Debezium 3.6.0.Final (1 Jul 2026) — https://github.com/debezium/debezium/blob/main/CHANGELOG.md
- Open Policy Agent 1.20.1 — https://github.com/open-policy-agent/opa/blob/main/CHANGELOG.md ; partial evaluation / data filtering — https://www.openpolicyagent.org/docs/filtering/partial-evaluation `[UNVERIFIED — host blocked]`
- MinIO community repository status ("THIS REPOSITORY IS NO LONGER MAINTAINED"; "Source-Only Distribution"; LICENSE = AGPLv3) — https://github.com/minio/minio/blob/master/README.md ; trade-press dating of the December 2025 maintenance-mode and 12 February 2026 archive steps — https://linuxiac.com/minio-ends-active-development/ `[UNVERIFIED — search-index excerpt]`
- SeaweedFS (Apache-2.0) — https://github.com/seaweedfs/seaweedfs ; Apache Ozone (Apache-2.0) — https://github.com/apache/ozone `[licence files read; suitability not assessed]`

## Modernization ledger (pass 2, 2026-09-03)

**What changed.**

- §7 "The front end" restated in the signal-first, zoneless Angular 22 idiom: `httpResource`/`resource` for every read, a `CatalogStore` SignalStore (`withEntities`, `withComputed`, `withMethods`, `patchState`) for global fabric state, standalone `OnPush` components with signal `input()`s, `@if`/`@for`/`@defer` control flow, Signal Forms for the query editor, `provideHttpClient()` once at bootstrap; the smart/dumb split is named as superseded. Per-major (v19–v22) stability of every named API is stated for the Legacy Island re-pin.
- TL;DR bullet 6 and 7, §5.2 (Iceberg row) and §7 (first-release list): the MinIO assumption removed — the community repository declares itself no longer maintained and source-only (AGPLv3); the object-storage slot is re-opened with Apache-2.0 candidates named and tied to open question 1.
- §5.2 gained a "current majors" table for every version-bearing tool; §9 regrouped into concept vs implementation/idiom sources.

**Verified against (all fetched 2026-09-03 via `raw.githubusercontent.com`, `registry.npmjs.org`, `pypi.org`).**

- Angular: `adev/src/content/guide/signals/resource.md`, `guide/http/http-resource.md`, `guide/zoneless.md` ("Zoneless is the default in Angular v21+"), `guide/forms/signals/overview.md` ("Signal Forms require: Angular v21 or higher"), `guide/components/inputs.md`; `CHANGELOG.md` — 22.0.0 (2026-06-03): "Set default Component changeDetection strategy to OnPush", "graduate signal forms APIs to public API", "Use `FetchBackend` as default"; 21.0.0 (2025-11-19): "Add migration for zoneless by default"; 20.0.0 (2025-05-28): "Promote zoneless to stable", "stabilize linkedSignal API"; 19.2.0 (2025-02-26): "introduce experimental `httpResource`"; 19.0.0 (2024-11-19): "mark input, output and model APIs as stable"; source JSDoc `@publicApi 22.0` on `resource`, `httpResource`, `rxResource`.
- npm: `@angular/core` 22.1.5, `@ngrx/signals` 22.0.0 (peer `@angular/core ^22.0.0`), `@astrouxds/angular` 9.0.0.
- NgRx: guide pages `signal-store/{index,linked-state,events,custom-store-features}.md` exist on `main`; `modules/signals/src/with-{linked-state,feature,props,hooks}.ts` and the `entities`/`events` entry points present.
- Tools: OpenMetadata `2.0.1-release` tag + PyPI; DataHub `v1.7.0` tag + PyPI; Trino `release-483.md`; Iceberg `site/mkdocs.yml` (`icebergVersion: '1.11.0'`) + `site/docs/releases.md`; Debezium `CHANGELOG.md`; OPA `CHANGELOG.md` + `v1/version/version.go`; MinIO `README.md` + `LICENSE`; SeaweedFS and Apache Ozone `LICENSE`.

**Stayed (version-independent).** Gartner/Forrester definitions, Dehghani, Fowler's IntegrationDatabase, the Lakehouse and Knowledge Graphs papers, all DoD/Army/JADC2/Zero Trust material, the four-commitments definition, the comparison table, the reference architecture and layer table, the DDD relationship, the trade-offs, the RR lens apart from its front-end paragraph, and the open questions.

**Still `[UNVERIFIED]`.** Everything so marked in pass 1 (gartner.com, martinfowler.com, media.defense.gov, api.army.mil, dni.gov, vendor and project-doc sites — still egress-blocked); MinIO's maintenance-mode and archive dates (trade press only); OPA 1.20.x release dates (the changelog carries none) and the Rego-v1-default claim; Ceph RGW / Ozone / SeaweedFS suitability as the S3 slot (licences read, nothing else); Ranger and Atlas current versions; DataHub and OpenMetadata Helm chart versions; OpenTDF Helm packaging.
