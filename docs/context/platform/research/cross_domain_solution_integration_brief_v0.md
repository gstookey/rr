...
| **NCDSMO** | NSA's National Cross Domain Strategy & Management Office — keeps the **baseline list** of approved CDS and runs **Raise the Bar (RTB)**, the post-2018 hardening strategy for CDS design and deployment `[UNVERIFIED — dates]`. |
...
- NSA NCDSMO public page and the *Raise the Bar* overview ([nsa.gov](https://www.nsa.gov/Resources/Everyone/Cross-Domain-Solutions/) `[UNVERIFIED — host blocked]`).
...
- OpenTDF specification — "an Open, Interoperable, JSON encoded data format for implementing Data Centric Security," modernising the IC's original XML-based TDF; manifest + encrypted payload; ABAC policy with `dataAttributes` and optional `dissem`; **handling assertions** "used for security labeling or handling instructions," cryptographically bound so they cannot be moved to another TDF. [OpenTDF, *spec*](https://github.com/opentdf/spec) and [OpenTDF, *platform*](https://github.com/opentdf/platform) (verified; BSD-3-Clause-Clear).
...
**Marking display is data-driven.** The banner component takes a marking object and a *display vocabulary* (colour, ordering, abbreviations, portion-mark syntax) loaded at runtime; on the low side the vocabulary contains only unclassified/CUI entries and test values; the high side ships a fuller vocabulary as config. The component logic — precedence, portion marks, banner top/bottom, print header — is identical and unit-tested against fixtures.

**Provenance in the UI.** Data that crossed a domain is rendered with an origin affordance (an AstroUXDS status/tag primitive: source domain, transfer time, "pending acknowledgement"), and stale-because-async is a first-class state, not an error.

**Feature flags per domain.** Flags are config, resolved at boot from the gateway's `/api/config`; a flag *key* is public, its *value* is per-domain. Flag names must themselves be unclassified.
...
- **Building / Floor / Suite / Office.** Domain = Building-level config; markings vocabulary and flags flow down; a Suite/Office never learns a marking string from code, only from the store (NgRx SignalStore slot for `domainConfig`, hydrated once from `/api/config`).
...
- NIST, *SP 800-53 Rev 5.2.0 — OSCAL catalog (controls AC-4, AC-16, CA-3, SC-7, SC-32, SC-39)*, 2026 — https://github.com/usnistgov/oscal-content (verified; raw JSON at `nist.gov/SP800-53/rev5/json/`).
- OpenTDF, *Trusted Data Format specification* — https://github.com/opentdf/spec (verified); *OpenTDF platform* — https://github.com/opentdf/platform (verified).
...
- NSA NCDSMO, *Cross Domain Solutions / Raise the Bar* — https://www.nsa.gov/Resources/Everyone/Cross-Domain-Solutions/ `[UNVERIFIED]`.
