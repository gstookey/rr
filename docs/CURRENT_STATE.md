...
- **ACME Workshop S0 (S-18) BUILT on branch `claude/acme-workshop-s0-foundation`, PR pending Graham** (2026-09-04): npm-workspaces layout (ADR-006), Angular 22.1.5 zoneless shell, nine `@rr/*` packages, Sheriff fences with a break-and-restore proof, 14-step local gate green, docker-compose Keycloak 26.7.3 + Postgres 18.6 with realm-as-code (NOT run here — no daemon; Graham verifies), mock OIDC, fictional seed. Kepler built · Verin APPROVE WITH NITS (nits fixed) · Vera VALIDATED. Nothing is implementation truth until it is on `main`.
