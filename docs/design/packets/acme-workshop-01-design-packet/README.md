...
5. **Name (revised the same hour, after the Invent/inventory collision was flagged):** **ACME Workshop** is the Building (`acme-workshop.com/`); Floors **Invent** (inventory, `/invent`), **Command** (tasking, `/command`), **Vigilance** (situational awareness, `/vigilance`), plus Axium's addition **Front Desk** (identity and delegated admin, `/front-desk`). Lexicon rules pinned: *Invent* is the inventory Floor and nothing else; **Command** (capitalised) is the Floor, *command* (lowercase) is a CQRS write message, and the thing a Campaign pushes to a device is an **instruction**, never a "command"; *Vigilance* is the Floor and a device's reported state is its *health*, never its "status".

## The Building, its Floors, and what each proves

| Tier | Name | Bounded context | Suites (v0) | Offices (v0) | Proves |
|---|---|---|---|---|---|
| L1 | **ACME Workshop** (the Building, `acme-workshop.com/`) | platform — shell, lobby, session, `@rr/*` base | — | Lobby · Sign-in · the Elevator | the unclassified base library; claims-driven lobby; markings from data |
| L2 | **Front Desk** (`/front-desk`) | Identity & Access (generic) | People · Groups | *Manage my group* (delegated admin) | delegated group admin via Keycloak FGAP, scoped, non-escalating |
| L2 | **Invent** (`/invent`) | Inventory (core) | Catalog · Devices | *Add product* · *Edit specs* · *Register device* · *Device inspector* (window) | the first vertical slice: read model + command + RLS + markings; new device *classes* without code (rung 1) |
| L2 | **Command** (`/command`) | Device Tasking (core) | Campaigns · Vectors · Entitlements | *Create campaign* · *Push update* · *Activate feature* · *Configure vectors* | commands crossing contexts by event; process-as-data (rung 2) for approval steps that differ per manufacturer; the paywall as an entitlement policy (rung 3) |
| L2 | **Vigilance** (`/vigilance`) | Situational Awareness (core) | Fleet · Map | *Fleet board* · *Device on map* (Cesium) · *Offline devices* | the live read model over SSE; the map; a B2B customer sees only its own operated devices |
...
flowchart LR
  ...
  W["Invent<br/>Inventory (core)<br/>products · specs · device registry"]
  C["Command<br/>Device Tasking (core)<br/>campaigns · vectors · entitlements"]
  S["Vigilance<br/>Situational Awareness (core)<br/>fleet · positions · health"]
...
Same-word traps already visible in this domain and pinned in `domain_model_v0.md`: **device** (a product model vs a serialised unit), **update** (software update vs record edit), **activate** (feature activation vs device activation), **customer** (a manufacturer as ACME's tenant vs the manufacturer's B2B customer), **command** (the Command Floor vs a CQRS command vs a device instruction), **status** (a generic word — the SA Floor is Vigilance and a device's state is its *health*).
...
- Fictional data only; no real markings; no external network at runtime (Cesium without ion — DA/AW-D1); nothing that would be a problem to port up.
