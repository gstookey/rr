...
> **B11 ANSWERED by Graham, 2026-09-03, before the questionnaire was ever sent:** (1) **Yes — Nexus holds the `@my-team/*` metadata** (and the `@ssd_victor/*` tree updates regularly there; `@other-team/*` is upgraded independently by its owning team). (2) **Linux — RHEL 9** on his island workstation: the bundles' linux-x64 platform filter is correct. (3) **Nothing runs puppeteer**, and every island app's `.npmrc` already carries `PUPPETEER_SKIP_DOWNLOAD=true`. Kept below for the record and for any second network owner. One follow-on check remains live: export what Nexus already serves and run the bundle's `--delta-from` against it (see the upload instructions — probably shrinks the Milestone-1 port to ~87 MB).
